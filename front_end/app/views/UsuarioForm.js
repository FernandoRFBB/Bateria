...
import { StyleSheet, TouchableOpacity, View, Text, ScrollView, Image, KeyboardAvoidingView, Platform } from "react-native";
...
export default function UsuarioForm ({ navigation, route }) {
    ...
    const [ escolas ] = useState([
        'Academicos do Bitt', 'Amigos do Bittencourt', 'Fefo123', 'Escola Zaccarias',
    ])
    const [ instrumentos ] = useState([
        'Caixa', 'Repique', 'Surdo 1', 'Surdo 2',
    ])

    const onSubmit = data => {
        console.log(data);
        navigation.navigate("Home", { created: true});
      }
    ...
    return (
        <ScrollView style={{marginTop: 10}}>
            ...
            <View>
              ...
              <View>
                <Controller
                  ...
                  render={({onChange, value}) => (
                    <Select
                      error={errors.escola}
                      placeholder={{label: "Escola", value: ""}}
              ...
              <View>
                <TouchableOpacity style={[styles.button, {marginTop: 10}]} onPress={handleSubmit(onSubmit)}>
